...
# make required os tree files
mkdir -p "${SYSEXTNAME}"/usr/bin/ "${SYSEXTNAME}"/usr/lib/systemd/system/ "${SYSEXTNAME}"/usr/lib/ "${SYSEXTNAME}"/etc/ "${SYSEXTNAME}"/proc/ "${SYSEXTNAME}"/sys/ "${SYSEXTNAME}"/dev/ "${SYSEXTNAME}"/run/ "${SYSEXTNAME}"/tmp/ "${SYSEXTNAME}"/var/tmp/
touch "${SYSEXTNAME}"/etc/resolv.conf
touch "${SYSEXTNAME}"/etc/machine-id
cp /usr/lib/os-release "${SYSEXTNAME}"/usr/lib/
cp /usr/bin/mkdir "${SYSEXTNAME}"/usr/bin/
...
if [ "${ONLY_CONTAINERD}" != 1 ]; then
  cat > "${SYSEXTNAME}/usr/lib/systemd/system/udocker.socket" <<-'EOF'
	[Unit]
	PartOf=udocker.service
...
EOF
  ...
  { echo "[Unit]"; echo "Upholds=udocker.socket"; } > "${SYSEXTNAME}/usr/lib/systemd/system/sockets.target.d/10-udocker-socket.conf"
  cat > "${SYSEXTNAME}/usr/lib/systemd/system/udocker.service" <<-'EOF'
	[Unit]
	Description=Docker Application Container Engine
	After=udocker-containerd.service udocker.socket network-online.target
	Wants=network-online.target
	Requires=udocker-containerd.service udocker.socket
	[Service]
	Type=notify
...
if [ "${ONLY_DOCKER}" != 1 ]; then
  cat > "${SYSEXTNAME}/usr/lib/systemd/system/udocker-containerd.service" <<-'EOF'
	[Unit]
...
EOF
  ...
  { echo "[Unit]"; echo "Upholds=udocker-containerd.service"; } > "${SYSEXTNAME}/usr/lib/systemd/system/multi-user.target.d/10-udocker-containerd-service.conf"
